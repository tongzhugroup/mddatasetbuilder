...
def qmcalc(gjfdir, command="g16", cpu_num: Optional[int]=None):
    ...
    GaussianRunner(command=command, cpu_num=cpu_num).runGaussianInParallel("GJF", gjflist)
...
def _commandline():
    ...
    parser.add_argument(
        "-n", "--ncpus", help="Number of CPU cores used, if not set, all CPU cores are used", default=None
    )
